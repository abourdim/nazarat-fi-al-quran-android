#!/usr/bin/env bash
# ============================================================================
#  Nazarat fi al-Quran Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/40-nazarat-fi-al-quran}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.nazaratfialquran}"
AVD_NAME="${AVD_NAME:-nazarat_fi_al_quran_test}"
...
cmd_test() {
  ...
  local out="/tmp/nazarat-fi-al-quran-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Nazarat fi al-Quran Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
